...
static int access_impl(const char *path, int mode) {
  if (path && asset_pack_stat_path(path, NULL, NULL)) return 0;
...
/* A packed directory has no fsdev DIR behind it, so opendir/closedir have to
 * know about the pack too -- readdir_fake already does. Without these the
 * engine's directory walks come back empty once the loose tree is gone. */
static void *opendir_pack(const char *path) {
  void *pd = asset_pack_opendir_path(path);
  if (pd) return pd;
  return (void *)opendir(path);
}
static int closedir_pack(void *dirp) {
...
static EGLBoolean egl_SwapBuffers_log(EGLDisplay d, EGLSurface s) {
   ...
   * eglSwapBuffers and the swapchain state logged beside it says why. */
  {
    ...
    bp_tate_swap_hook();   /* inert here: BP_TATE_ENABLE 0 (landscape) */
    battd_video_draw();    /* splash clip over the frame, before the swap.
                            * GL only -- the managed side of the splash is
                            * pumped from bp_boot.c, on the thread that runs
                            * managed code. See battd_il2cpp_pump(). */
...
DynLibFunction dynlib_functions[] = {
  ...
  { "opendir", (uintptr_t)&opendir_pack }, { "closedir", (uintptr_t)&closedir_pack },

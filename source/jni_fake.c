...
#include "battd_video.h"  /* battd_video_mix_audio: splash audio into the same block */
#include "editbox.h"
...
/* ---- third-party SDK classes (battd_nx) --------------------------------------
...
static const char *const k_sdk_null_prefix[] = {
  /* Ad networks and mediation, from the APK's own dex (21040 types scanned):
   * IronSource is the mediator; AppLovin (with its bundled ExoPlayer), AdColony,
   * Unity Ads/Services, Amazon DTB and Vungle are adapters behind it. */
  "com/ironsource/", "com/applovin/", "com/adcolony/", "com/unity3d/ads/",
  "com/unity3d/services/", "com/unity3d/scar/", "com/amazon/device/",
  "com/vungle/", "com/iab/omid/", "com/adjust/", "com/appsflyer/",
  /* Social, stores, analytics, web view, push */
  "com/facebook/", "com/android/billingclient/", "com/google/android/play/",
  "com/google/games/", "com/google/android/gms/ads", "com/google/ads/",
  "com/google/firebase/", "com/crashlytics/", "io/fabric/",
  "com/onevcat/uniwebview", "com/unity/androidnotifications",
  "org/chromium/net",
  /* Bloons Pop had Kongregate here and it is what crashed that port's first
   * hardware run. BATTD has no Kongregate; the prefix is left out rather than
   * carried over, so an unexpected hit shows up in the ledger instead of being
   * silently absorbed. */
  NULL };
...
static void *fmod_audio_thread(void *arg) {
  ...
  for (;;) {
    ...
    do {
      process(fake_env, &g_fmod_this_token, &g_fmod_bb_token);
      /* Splash-video audio rides the block FMOD just filled, rather than a
       * second SDL device. No-op unless a clip is decoding, and it never logs
       * or touches the filesystem -- this is the audio pump thread. */
      if (frame_bytes > 0)
        battd_video_mix_audio((short *)g_fmod_staging, block / frame_bytes, channels);
